...
def test(args, device, epoch, net, closerloader, openloader, threshold=0):
    ...
    with torch.no_grad():
        ...
        unk_mask = targets_list == args.known_class

        os_known_correct = np.sum((pred_list == targets_list) & known_mask)
        os_known_total = np.sum(known_mask)
        os_known_acc = os_known_correct / os_known_total * 100 if os_known_total > 0 else 0.

        # unknown accuracy: how many unknown samples are correctly predicted as unknown
        os_unk_correct = np.sum((pred_list == args.known_class) & unk_mask)
        os_unk_total = np.sum(unk_mask)
        os_unk_acc = os_unk_correct / os_unk_total * 100 if os_unk_total > 0 else 0.

        # HOS = harmonic mean of known and unknown accuracy
        if (os_known_acc + os_unk_acc) > 0:
            hos = 2 * os_known_acc * os_unk_acc / (os_known_acc + os_unk_acc)
        else:
            hos = 0.

        osda_result = {
            'os_known_acc': os_known_acc,
            'os_unk_acc': os_unk_acc,
            'HOS': hos
        }

    return osr_result, close_test_result, osda_result